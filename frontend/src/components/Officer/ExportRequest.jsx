...
function ExportRequest() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:7070/exportapp/all');
  ...
  const handleConfirmSave = () => {
    ...
    axios.put('http://localhost:7070/exportapp/updateStatus', null, {
      params: {
    ...
    .then(response => {
      ...
      if (newStatus === 'APPROVED') {
        ...
        axios.post("http://localhost:7070/invoice", formData, {
          headers: {
      ...
      <div className="table-container mb-4">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item, index) => (
                <tr key={item.applicationId}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{item.exporterName}</td>
        ...
        <div className="mt-4 flex justify-between items-center">
          <div>
            ...
            <button
              onClick={() => setPage(Math.min(page + 1, Math.ceil(filteredData.length / rowsPerPage) - 1))}
